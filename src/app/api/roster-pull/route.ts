 ...
 * "Anyone with the link." Only active once both env vars are set; see
 * the README note near the bottom of this file for the one-time setup.
 */
 ...
 * link — Viewer." Kept so the button still does something useful before
 * the service account is set up.
 */
...
export async function GET() {
  ...
  try {
    const buffer =
      email && key ? await fetchViaServiceAccount(email, key) : await fetchViaPublicExport();
    ...
    const message =
      email && key
        ...
        : 'לא ניתן לגשת לגיליון. יש לשתף אותו ב-Google Sheets כ"כל מי שיש לו את הקישור – צפייה" (כפתור שיתוף בפינה הימנית העליונה), או להגדיר חשבון שירות (ראו הערה ב-route.ts).';
...
//    then redeploy. Nothing else changes; this route picks the
